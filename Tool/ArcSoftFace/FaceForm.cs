...
namespace ArcSoftFace
{
    public partial class FaceForm : Form
    {
        ...
        public FaceForm()
        {
            InitializeComponent();
            InitEngines();
            this.pictureBox1.Parent = this.picImageCompare;
            OnSwitch(true);
            var interval = TimeSpan.FromMilliseconds(50);
            Task.Run(() => this.TryReadCameraLoop(interval));
        }
        ...
        private void TryReadCameraLoop(TimeSpan interval)
        {
            while (true)
            {
                try
        ...
        /// 读取摄像头图片
        /// </summary>
        private void ReadImageAndRaise()
        {
            if (this.capture == null)
            {
                return;
            }
            using (var img = this.capture.QueryFrame())
            {
                var act = new Action<Image>(FaceIdentify);
                Invoke(act, img.Bitmap);
            }
        }
        ...
        private void OnSwitch(bool value)
        {
            try
            {
                ...
                if (value == true)
                {
                    ...
                    this.capture.FlipType = FLIP.HORIZONTAL;
                    this.capture.SetCaptureProperty(CAP_PROP.CV_CAP_PROP_FRAME_WIDTH, 1870);
                    this.capture.SetCaptureProperty(CAP_PROP.CV_CAP_PROP_FRAME_HEIGHT, 920);
                }
            }
            catch (Exception ex)
            {
        ...
        {
            if ((e.KeyCode == Keys.Q && e.Control) || e.KeyCode == Keys.Escape)
            {
                Application.Exit();
            }
        }
